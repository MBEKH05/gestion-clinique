...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (LoginRateLimiter::isLocked($identifier)) {
            ...
            Log::channel('security')->warning("Login blocked (lockout active) for {$identifier}");
        ...
        if (! $token = Auth::guard('api')->attempt([
            ...
            Log::channel('security')->warning("Failed login attempt for username '{$credentials['username']}' from {$identifier}");
