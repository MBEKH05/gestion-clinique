...
class LabAuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (LoginRateLimiter::isLocked($identifier)) {
            ...
            Log::channel('security')->warning("Lab login blocked (lockout active) for {$identifier}");
        ...
        if (! $token = Auth::guard('lab')->attempt([
            'username' => $credentials['username'],
            'password' => $credentials['password'],
        ])) {
            LoginRateLimiter::hit($identifier);
            Log::channel('security')->warning("Failed lab login attempt for username '{$credentials['username']}' from {$identifier}");
    ...
    public function check(Request $request)
    {
        return response()->json([
            'authenticated' => true,
            'user' => $request->user('lab')->toUserArray(),
        ]);
    }
    ...
    public function refresh(Request $request)
    {
        ...
        $user = LabUser::find($payload->get('sub'));
