...
export default function IPMTarifs() {
  const { id } = useParams();
  const { ipms, analyses, tarifs, categories, addTarif, updateTarif, deleteTarif } = useData();
  ...
  const tarifsParAnalyse = useMemo(() => {
    const map = new Map();
    tarifs
      .filter((t) => t.typePriseEnCharge === 'IPM')
      .forEach((t) => map.set(t.analyseId, t));
    return map;
  }, [tarifs]);
  ...
  const handleAdd = async (e) => {
    ...
    await addTarif({ analyseId: newAnalyseId, typePriseEnCharge: 'IPM', prix: Number(newPrix) });
    setNewAnalyseId('');
    setNewPrix('');
  };
  ...
  const handleEditPrix = async (analyseId, tarif) => {
    ...
    if (tarif) {
      ...
      await addTarif({ analyseId, typePriseEnCharge: 'IPM', prix });
    }
  };
  ...
  if (!ipm) {
    return <div className="alert alert-warning">IPM introuvable.</div>;
  }

  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="mb-0">Tarifs IPM &mdash; {ipm.nom}</h2>
        <Link to="/ipm" className="btn btn-outline-secondary">
          Retour
        </Link>
      </div>

      <div className="alert alert-info">
        Les tarifs IPM sont generiques : ils s'appliquent a toutes les IPM.
      </div>
